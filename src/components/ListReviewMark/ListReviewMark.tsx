...
export const ListReviewMark = () => {
  ...
  const fetchTitleClass = async () => {
    const title = await classroomApi.getClassByCodeClass({
      jwt: localStorage.getItem('jwt'),
      codeclass: codeclass,
    });
            ...
            <div className={`${className}`}>
              <div className={`${className}__header`}>
                <div className={`${className}__left`}>
                  ...
                  <h4 className={`${className}__text-primary`}>{titleClass}</h4>
                </div>
              </div>
